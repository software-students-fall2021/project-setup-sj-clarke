...
function Friends() {
  ...
  const friend = [{ username: "sjclarke10" }, { username: "jDoe1" }];

  const inputAddFriend = ["Username "];

  const inputChargeFriend = ["Expense Amount "];

  const inputAddGroup = ["Group name "];
  ...
  return (
    <div className="Friends">
      ...
      <ReactBootStrap.Table striped bordered hover>
        ...
        <tbody>{friend.map(renderRow)}</tbody>
